...
\begin{document}
	...
	\begin{figure}
		...
		\node (c) at (5.5, -1.8) {\scriptsize Erased because connected component too small};
		\draw[->, thick, gray] (c.west) -- (0.98, -0.35);
		\node (d) at (-0.15, -1.65) {\scriptsize Erased because the intensity is too low};
	...
	\begin{figure}[h]
		\centering
		\begin{tikzpicture}
		\node[] (a) at (0, 0) {\includegraphics[scale=2.9]{sobel_ws.png}};
		\node[] (b) at (0, -2.7) {\includegraphics[scale=2.9]{sobel_s.png}};
		\node[below=2.5mm, white] at (a.150) {\small \, Without smoothing};
		\node[below=2.5mm, white] at (b.150) {\small With smoothing};
		\end{tikzpicture}
		\vspace{-3mm}
